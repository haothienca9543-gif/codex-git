...
def extract_region(html: str) -> str | None:
    match = re.search(r'"region":"([A-Z]{2})"', html)
    if match:
    ...
    if next_data_match:
        payload = json.loads(next_data_match.group(1))
        candidate = _find_key(payload, "region")
...
def main() -> int:
    ...
    print(f"Thời gian đăng ({args.timezone}): {pretty_time}")

    if region:
        country_timezone = COUNTRY_TIMEZONES.get(region)
        if country_timezone:
            try:
                country_time = format_timestamp(create_time, country_timezone)
                print(f"Giờ đăng theo quốc gia TikTok ({region} - {country_timezone}): {country_time}")
            except ValueError as exc:
                print(f"Không thể hiển thị giờ theo quốc gia TikTok ({region}): {exc}")
        else:
            print(f"Không có mapping múi giờ cho quốc gia TikTok: {region}")
    else:
        print("Không xác định được quốc gia TikTok từ dữ liệu trang.")
